...
from PyQt5.QtGui import QIcon, QPalette, QColor, QFont, QDragEnterEvent, QDropEvent
...
class LogLoaderThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Count total messages for progress (if possible)
            try:
                with open(self.file_path, 'rb') as f:
                    for _ in f:
                        total_messages += 1
            except:
                total_messages = 0  # Fallback if counting fails
            ...
            for msg in reader:
                ...
                if self.db:
                    try:
                        ...
                        print(f"Decoded {message.name} for ID 0x{msg.arbitration_id:X}: {decoded}")
            ...
            self.deleteLater()  # Clean up thread
        ...
        try:
            ...
            if self.time_range is None:
                ...
                for signal in self.selected_signals:
                    if signal in self.signals_data:
            ...
            for signal in self.selected_signals:
                if signal not in self.signals_data:
                    print(f"Signal {signal} not in signals_data")
                    continue
                data = self.signals_data[signal]
                print(f"Raw data for {signal}: {len(data)} rows")
                
                data = data[(data['timestamp'] >= start_time) & 
                           (data['timestamp'] <= end_time)]
                print(f"Filtered data for {signal}: {len(data)} rows")
...
class CANAnalyzerApp(QMainWindow):
    """
    CANAnalyzerApp is a PyQt-based GUI application for analyzing CAN (Controller Area Network) data. 
    It provides functionalities to load DBC files, CAN log files, and visualize signals through interactive plots.
    Class Attributes:
    -----------------
    - dbc_file : str
        Path to the loaded DBC file.
    - db : cantools.database.Database
        Parsed DBC database object.
    - log_file : str
        Path to the loaded CAN log file.
    - log_data : pandas.DataFrame
        DataFrame containing the parsed CAN log data.
    - signals_data : dict
        Dictionary containing signal data extracted from the log file.
    - is_dark_mode : bool
        Indicates whether the application is in dark mode.
    - status_history : list
        List of status messages for the status bar.
    Methods:
    --------
    - __init__():
        Initializes the main window and sets up the UI components.
    - init_ui():
        Sets up the user interface, including toolbars, sidebars, and main panels.
    - init_plot_area():
        Initializes the plot area with tabs for signal plots and raw data.
    - add_plot_tab(title: str):
        Adds a new tab for plotting signals.
    - close_tab(index: int):
        Closes a plot tab by index.
    - switch_view(item: QListWidgetItem):
        Switches to the tab corresponding to the selected view in the sidebar.
    - dragEnterEvent(event: QDragEnterEvent):
        Handles drag-and-drop events for file loading.
    - dragLeaveEvent(event: QDragLeaveEvent):
        Resets the window style when a drag operation leaves the window.
    - dropEvent(event: QDropEvent):
        Handles file drop events to load DBC or log files.
    - toggle_theme():
    ...
    - show_message_context_menu(position: QPoint):
        Displays a context menu for the message list.
    - show_signal_context_menu(position: QPoint):
        Displays a context menu for the signal list.
    - load_dbc_file(file_path: str = None):
        Loads a DBC file and parses it.
    - clear_dbc():
        Clears the loaded DBC file and resets related UI components.
    - load_log_file(file_path: str = None):
        Loads a CAN log file and parses it.
    - clear_log():
        Clears the loaded log file and resets related UI components.
    - on_log_loaded(df: pandas.DataFrame, signals_data: dict):
        Callback for when the log file is successfully loaded.
    - on_log_error(error: str):
        Callback for when an error occurs during log file loading.
    - populate_message_list():
        Populates the message list with messages from the DBC or log file.
    - update_signal_list():
        Updates the signal list based on the selected messages.
    - clear_messages():
        Clears the message list.
    - clear_signals():
        Clears the signal list.
    - populate_raw_data_table():
        Populates the raw data table with log data.
    - filter_raw_data():
        Filters the raw data table based on user input.
    - export_raw_data():
        Exports the raw data table to a CSV file.
    - filter_messages():
        Filters the message list based on user input.
    - filter_signals():
        Filters the signal list based on user input.
    - toggle_time_range(text: str):
        Toggles the time range input fields based on the selected option.
    - plot_signals(update: bool = False):
        Plots the selected signals on a new or existing tab.
    - on_plot_finished(plot_data: dict, fig: matplotlib.figure.Figure, canvas: FigureCanvas, stats_label: QLabel):
        Callback for when the plotting process is complete.
    - export_plot():
        Exports the current plot or data to a file.
    """
    def __init__(self):
        super().__init__()
        ...
        if self.db:
            for item in selected_items:
                ...
                try:
                    ...
                    for signal in message.signals:
                        self.signal_list.addItem(f"{message.name}.{signal.name}")
                        signals_found = True
                except KeyError:
            ...
            for item in selected_items:
                ...
                for i in range(max_dlc):
                    self.signal_list.addItem(f"ID_0x{arb_id:X}.Byte{i}")
                    signals_found = True
    ...
    def plot_signals(self, update=False):
        ...
        valid_signals = [s for s in selected_signals if s in self.signals_data]
        if not valid_signals:
            QMessageBox.warning(self, "Warning", f"No data available for selected signals! Available signals: {list(self.signals_data.keys())}")
            return
        ...
        if self.time_range_combo.currentText() == "Custom":
            ...
            if start_time >= end_time:
                ...
                return
            # Check if time range contains data
            has_data = False
            for signal in valid_signals:
                data = self.signals_data[signal]
                if not data[(data['timestamp'] >= start_time) & (data['timestamp'] <= end_time)].empty:
                    has_data = True
                    break
            if not has_data:
                QMessageBox.warning(self, "Warning", "No data points in the selected time range!")
                return
            time_range = (start_time, end_time)
        
        channel = None  # channel_combo not defined in UI, default to None
        channel_filter = None
        
        self.update_status("Plotting signals...")
        self.plotter_thread = PlotterThread(
            self.signals_data, selected_signals, time_range, 
